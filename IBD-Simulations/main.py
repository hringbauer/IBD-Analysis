...
growing = 1  # Wether to use growing model or not
...
def main():
    ...
    while True:
        ...
        if inp == 1:
            ...
            #position_list = [(50 + i * 2, 50 + j * 2, 0) for i 
            #                 in range(12) for j in range(5)]    # Introduced this for grant
            #grid.set_samples(position_list)
            
            grid.set_random_samples(500)    # Sets random samples
                
        if inp == 2:     
            if grid == 0:  # Make new grid if not existing
                grid = factory_Grid(growing)  # Factory method to produce Grid or Growing_grid class         
