...
Contains the machinery to create plots
@author: Harald
'''
...
def plot_sharing_times():
    ...
    axarr[0].set_title("Block sharing for 1/T model", fontsize=32)
    # plt.tight_layout()
    plt.show()
...
def plot_IBD_curves():
    '''Method that plots curves of expected IBD'''
    l=0.05
    r_vec = np.linspace(1, 20, 1000)
    y_vec=r_vec**2 * kv(2,np.sqrt(2*l)*r_vec)
    
    plt.figure()
    plt.plot(r_vec, y_vec, 'r-', label="High Density", linewidth=4)
    plt.plot(r_vec, 0.5*y_vec, 'm-', label="Medium Density", linewidth=4)
    plt.plot(r_vec, 0.2*y_vec, 'b-', label="Low Density", linewidth=4)
    plt.yscale("log")
    plt.ylabel("Shared IBD blocks (5 cM)", fontsize=20)
    plt.xlabel(r"Pairwise Distance $[\sigma]$", fontsize=20)
    plt.xticks([])
    plt.yticks([])
