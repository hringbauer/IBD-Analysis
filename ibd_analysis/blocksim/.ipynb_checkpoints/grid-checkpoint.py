...
#############################################################################
    ...
    def create_MLE_object(self, bin_pairs=False, min_dist=0, reduce_start_list=False, plot=False):
        ...
        pair_dist[pair_dist == 0] = 0.001  # Set minimal pw. distance. To avoid numerical instability for identical pairs
    ...
    def give_lin_IBD(self, bin_pairs=False, reduce_start_list=False, min_dist=0):
        ...
        Used as input for full MLE-Method. 
        bin_pairs: Pool pairs with same distances. (and calculate respective number of pairs) 
        ...
        min_dist: minimal pw. distance used in analysis
        ...
        if bin_pairs:  # Pool data if wanted (speeds up MLE)
...
class Grid_Heterogeneous(Grid):
    ...
    def update_t(self, t):
        ...
        start = timer()
        for i in range(0, t):
            print("Doing step: " + str(i) + "/" + str(t))
            self.set_chr_pn(self.t + 1)  # Set Nr of individuals per node t generations back
            print("IIDs left: " + str(self.nr_inds_left))
            print("IIDs right: " + str(self.nr_inds_right))
