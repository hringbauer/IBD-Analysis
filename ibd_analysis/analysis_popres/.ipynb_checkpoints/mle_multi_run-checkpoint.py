...
class MLE_analyse(object):
    ...
    def __init__(self, data=0, pw_dist=[], pw_IBD=[], pw_nr=[], all_chrom=False, 
                 error_model=True, position_list=[]):
        '''
        Constructor. If data do the POPRES data processing (including ctry matrices, 
        Else take the three vectors:
        pw_dist: List of pw. geographic distances
        pw_IBD: List of pw. IBD between sample coordinates
        pw_nr: List of pw. comparisons between sample coordinates
        position_list: Needed for heterogeneous Migration/Pop. Density Patterns!
        Warning: Chrom Length  specified in Morgan!
        '''
    ...
    def create_mle_model_barrier(self, model="hetero", g=3537.4, start_param=[], diploid=0,
        ...
        elif model == "hetero_Ne.fix":
            start_params = np.array([150, 150, 0.5, 0.5])
            if len(start_param) > 0:
                start_params = start_param
...
def selfing_density_poisson_uniform(l, r, params, g, s=0.9115):
